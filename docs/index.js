$(document).ready(function () {
  ...
  if (NowWeek != null) {
    document.querySelector("#simulatorScreen").classList.remove("d-none");
    document.querySelector("#engineButtons").classList.remove("d-none");
    var lastWeek = NowWeek - 1;
    if (NowWeek != 1) {
      document.querySelector("#finishLeague").classList.remove("d-none");
...
function PrintFixtureTable(week) {
  ...
  updateLeagueTable();
  document.querySelector("#weekNumber").innerHTML =
    String(parseInt(week)) + ". Week";
}
...
function whenLeagueEndUpdateButtons() {
  ...
  $("#champmionAlert").removeClass("d-none");
  $("#champmionAlert").html("CHAMPION " + Teams[0].Name);
}
...
function championshipChance() {
  ...
  if (NowWeek == 4) {
    ...
    compareTeamsChance(
      ...
      third,
      fourth,
      requiredPointsDifferenceForChampionship
    );
  }
  if (NowWeek == 5) {
...
function appendChampionshipChance(Teams) {
  for (i = 0; 4 > i; i++) {
    $("#championsChance").append(
      "<tr><td>" +
        Teams[i].Name +
        "</td><td>" +
        Teams[i].PredictionsOfChampionship +
        "</td></tr>"
    );
  }
}
...
function removeStartButtonDisplayResteButton() {
  ...
  $(".page-header").on("click", "#resetButton", function () {
    var res = confirm("Are you sure? All data will be lost.");
    if (res == true) {
      resetProgram();
    }
  });
...
function compareTeamsChance(
  ...
  if (firstVsSecond == 0) {
    second.PredictionsOfChampionship = "%60";
    first.PredictionsOfChampionship = "%60";
  } else if (firstVsSecond > gerekliPuanFarki) {
    second.PredictionsOfChampionship = "%0";
    first.PredictionsOfChampionship = "%100";
  } else if (firstVsSecond < gerekliPuanFarki) {
    second.PredictionsOfChampionship = "%60";
    first.PredictionsOfChampionship = "%60";
  } else if (firstVsSecond == gerekliPuanFarki) {
    second.PredictionsOfChampionship = "%30";
    first.PredictionsOfChampionship = "%80";
  }

  if (firstVsThird == 0) {
    third.PredictionsOfChampionship = "%60";
  } else if (firstVsThird > gerekliPuanFarki) {
    third.PredictionsOfChampionship = "%0";
  } else if (firstVsThird < gerekliPuanFarki) {
    third.PredictionsOfChampionship = "%50";
  } else if (firstVsThird == gerekliPuanFarki) {
    third.PredictionsOfChampionship = "%30";
  }

  if (firstVsFourth == 0) {
    fourth.PredictionsOfChampionship = "%60";
  } else if (firstVsFourth > gerekliPuanFarki) {
    fourth.PredictionsOfChampionship = "%0";
  } else if (firstVsFourth < gerekliPuanFarki) {
    fourth.PredictionsOfChampionship = "%50";
  } else if (firstVsFourth == gerekliPuanFarki) {
    fourth.PredictionsOfChampionship = "%30";
  }
}
...
function updateMatchScore() {
  ...
  jQuery.each(Fixture, function (i, match) {
    if (
      match.Home == homeTeamUp &&
      match.Away == awayTeamUp &&
      match.Week == week
    ) {
      ...
      match.Home == homeTeamDown &&
      match.Away == awayTeamDown &&
      match.Week == week
    ) {
